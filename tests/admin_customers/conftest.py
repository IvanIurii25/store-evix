"""Admin-customers test fixtures (§6.2).

Mounts only the admin-customers router under ``/api/v1`` with ``get_session``
bound to the commit-safe ``db_session`` and ``current_staff`` overridden to a
fixed staff user. ``guest_client`` exercises the real guard.

Run with ``EVIX_TEST_DB=evix_test_admin_customers``.
"""
...
TEST_STAFF_ID: int = 8201


def _build_app(db_session: AsyncSession) -> FastAPI:
    """Assemble a minimal app mounting the admin-customers router."""
    app = FastAPI()
    register_exception_handlers(app)
    app.include_router(admin_customers_router, prefix=_API_V1_PREFIX)
